...
Note: The program performs its functions per gene, to construct two trees you have to run it two times one for the NA gene and one for the HA gene.
...
Run time: the run time varies with the number of sequences and length of sequences but it is estimated that  for a dataset of 20 sequences with 1700-1800 bp length, the run time is around 40 mins.
